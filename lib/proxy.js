...
var discardHeadersForCache = ['cache-control', 'expires', 'etag', 'last-modified', 'content-length'];
...
module.exports = function(options) {
    ...
    cache.exists(cacheKey, function(err, exists) {
      ...
      apiRequest.on('response', function(resp) {
        if (resp.statusCode >= 400) {
          ...
          for (key in resp.headers) {
            if (_.contains(discardHeadersForCache, key) === false)
              res.set(key, resp.headers[key]);
          }
  ...
  function buildOptions(req, options, originUrl) {
    ...
    // If there are global required options for this plugin, allow them to trump
    // whatever was passed in. Only relevant in a multi-tenant scenario where 
    // the host platform needs to enforce mandatory options that override what
    // an individual app owner passes in. In the case of the api proxy, such 
    // values might include cacheMaxAge and timeout.
    if (req.app.platformOptions && _.isObject(req.app.platformOptions['express-api-proxy']))
      _.extend(proxyOptions, req.app.platformOptions['express-api-proxy']);
