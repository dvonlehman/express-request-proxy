...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    if (method.toUpperCase() == 'GET' && options.cache === true && options.cacheMaxAge > 0) {
      if (!options.cache)
  ...
  function makeApiCall(req, res, next) {
    ...
    debug("making API call to %s", apiRequestOptions.url.host);
  ...
  function proxyViaCache(req, res, next) {
    ...
    if (_.isFunction(options.cacheKeyFn))
      ...
      cacheKey = formatUrl(proxyOptions.originUrl);
    ...
    options.cache.exists(cacheKey, function(err, exists) {
      ...
      apiRequest.on('response', function(resp) {
        if (resp.statusCode >= 400) {
          ...
          debug("piping response through cache and out to response");
          apiRequest
  ...
  function enforceLimits(req, options) {
    var limits = req.app.settings.platformLimits;
    if (_.isObject(limits) === false)
      return;

    if (_.isNumber(limits.maxNetworkTimeout)) {
      if (_.isNumber(options.timeout) === false || options.timeout > limits.maxNetworkTimeout)
        options.timeout = limits.maxNetworkTimeout;
    }
    if (_.isNumber(limits.maxHttpRedirects)) {
      if (_.isNumber(options.maxRedirects) === false || options.maxRedirects > limits.maxRedirects)
        options.maxRedirects = limits.maxHttpRedirects;
    }
  }
};
