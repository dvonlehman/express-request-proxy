...
var blockHeaders = ['host', 'cookie'];
var blockHeadersCache = ['if-none-match', 'if-modified-since'];
...
module.exports = function(req, options, limits) {
  ...
  function shouldPassthroughHeader(header) {
    if (_.contains(blockHeaders, header) === true) return false;
    if (options.cache && _.contains(blockHeadersCache, header) === true) return false;
