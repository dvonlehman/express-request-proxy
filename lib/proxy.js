...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    try {
      var proxyOptions = req.ext.apiProxyOptions = buildOptions(req, options, originUrl);
    }
    catch (err) {
      return next(Error.http(400, err.message));
    }

    if (proxyOptions.cache === 'memory') {
      proxyOptions.cache = new MemoryCache();
    }
    ...
    function makeApiCall() {
      ...
      if (_.isArray(proxyOptions.transforms))
        responseStream = applyTransforms(responseStream, proxyOptions.transforms);
