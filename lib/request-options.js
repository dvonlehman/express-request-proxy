...
module.exports = function(req, options, limits) {
  ...
  if (_.isObject(req.body)) {
    // Intentionally not using the json and form properties. By doing our own
    // serialization we can get the content-length and pass it as a header.
    var bodyText;
    if (req.get('Content-Type') === 'application/json') {
      ...
      bodyText = querystring.stringify(req.body);
    }

    requestOptions.body = bodyText;
    requestOptions.headers['Content-Length'] = bodyText.length;
  }
